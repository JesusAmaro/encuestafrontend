...
const orderQustions = (state: Poll) => {
...
export const PollReducer = produce((state: Poll, action: PollActions) : Poll => {
    switch(action.type) {
        ...
        case 'removeanswer':
            ...
            if(state.questions[action.payload.index].answers.length > MIN_ANSWERS_PER_QUESTION){
                state.questions[action.payload.index].answers = state.questions[action.payload.index].answers.filter( a => a.id !== answerId);
            }
            state.errors = {};
            return state;
